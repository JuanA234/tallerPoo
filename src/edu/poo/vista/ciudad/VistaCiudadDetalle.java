...
import edu.poo.controlador.ciudad.ControladorCiudadDetalle;
...
public class VistaCiudadDetalle extends SubScene {
    ...
    private Label lblNombreImagenDepartamento;
    ...
    public VistaCiudadDetalle(BorderPane princ, Pane pan, double anchoPanel, double altoPanel, int ind) {
...
//        Rectangle marco = Marco.crear(nuevoAncho, nuevoAlto, arrColores, colorBorde);
    ...
    private void construirPanelIzquierdo(double anchoPanel, double altoPanel) {
        ...
        btnAnterior.setOnAction((t) -> {
            indice = ControladorCiudadDetalle.rotar("anterior", indice, lblNombre,
                    lblNombreImagen, lblContador, lblNombreDepartamento, lblNombreImagenDepartamento, imagenMostrar, anchoPanel, altoPanel);
        });
    ...
    private void construirPanelDerecho(double anchoPanel, double altoPanel) {
        ...
        btnSiguiente.setOnAction((ActionEvent t) -> {
            indice = ControladorCiudadDetalle.rotar("siguiente", indice,
                    lblNombre, lblNombreImagen, lblContador, lblNombreDepartamento, lblNombreImagenDepartamento, imagenMostrar, anchoPanel, altoPanel);
        });
    ...
    private void construirPanelCentral(double anchoPanel, double altoPanel) {
        ...
        btnEliminar.setOnAction((t) -> {
            int cantidad = ControladorCiudad.obtenerCantidadDeCiudades();
            if (cantidad > 0) {
                ...
                if (msg.showAndWait().get() == ButtonType.OK) {
                    ControladorCiudad.eliminar(indice);
                    indice = ControladorCiudadDetalle.rotar("anterior", indice, lblNombre,
                            lblNombreImagen, lblContador, lblNombreDepartamento, lblNombreImagenDepartamento, imagenMostrar,
        ...
        btnActualizar.setOnAction((t) -> {
            int cantidad = ControladorCiudad.obtenerCantidadDeCiudades();
            if (cantidad > 0) {
                panelCuerpo = ControladorCiudadVentana.ciudadEditar(panelPrincipal, panelCuerpo,
        ...
        lblNombreDepartamento = new Label();
        fuente = Font.font("Verdana", FontWeight.BOLD, FontPosture.ITALIC, 15);
        lblNombreDepartamento.setFont(fuente);
        lblNombreDepartamento.setTextFill(Color.WHITESMOKE);

        HBox panelHorizontalNombre = new HBox(4);
        panelHorizontalNombre.setAlignment(Pos.CENTER);
        panelHorizontalNombre.getChildren().addAll(lblNombreDepartamento, lblNombre);
        ...
        lblNombreImagenDepartamento = new Label();
        lblNombreImagenDepartamento.setTextFill(Color.YELLOWGREEN);
        fuente = Font.font("Verdana", 12);
        lblNombreImagenDepartamento.setFont(fuente);

        HBox panelHorizontalNombreImagen = new HBox(4);
        panelHorizontalNombreImagen.setAlignment(Pos.BOTTOM_CENTER);
        panelHorizontalNombreImagen.getChildren().addAll(lblNombreImagen, lblNombreImagenDepartamento);
        ...
        ControladorCiudadDetalle.rotar("primero", indice, lblNombre,
                lblNombreImagen, lblContador, lblNombreDepartamento, lblNombreImagenDepartamento, imagenMostrar, anchoPanel, altoPanel);
